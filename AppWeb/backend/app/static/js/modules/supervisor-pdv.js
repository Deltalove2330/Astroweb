document.addEventListener('DOMContentLoaded', function() {
    ...
    let map = null;
    let marker = null;
    let direccionAutocomplete = null;
    ...
    function cargarListasDesplegables() {
        ...
        fetch('/supervisor/api/pdv/departamentos')
            .then(response => response.json())
            .then(data => {
                departamentos = data;
        ...
        // Cargar ciudades
        fetch('/supervisor/api/pdv/ciudades')
            .then(response => response.json())
            .then(data => {
                ciudades = data;
        ...
        fetch('/supervisor/api/pdv/jerarquias-n2')
            .then(response => response.json())
            .then(data => {
                jerarquiasN2 = data;
        ...
        fetch('/supervisor/api/pdv/jerarquias-n2-2')
            .then(response => response.json())
            .then(data => {
                jerarquiasN2_2 = data;
    ...
    function cargarCiudadesPorDepartamento() {
        ...
        fetch(`/supervisor/api/pdv/ciudades-por-departamento/${encodeURIComponent(departamento)}`)
            .then(response => response.json())
            .then(data => {
                actualizarSelect('ciudad', data);
            })
    ...
    function cargarJerarquiasN2_2PorN2() {
        ...
        fetch(`/supervisor/api/pdv/jerarquias-n2_2-por-n2/${encodeURIComponent(jerarquiaN2)}`)
            .then(response => response.json())
            .then(data => {
                actualizarSelect('jerarquia_n2_2', data);
            })
    ...
    function generarIdentificadorAutomatico() {
        ...
        if (!pdvId && jerarquiaN2_2) {
            ...
            fetch(`/supervisor/api/pdv/next-identificador/${encodeURIComponent(jerarquiaN2_2)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
    ...
    function inicializarAutocomplete() {
        ...
        sugerenciasContainer.className = 'autocomplete-suggestions';
        ...
        direccionInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(() => {
                buscarSugerenciasDireccion(query, sugerenciasContainer, direccionInput);
            }, 500);
        });
        
        // Ocultar sugerencias al hacer clic fuera
        document.addEventListener('click', function(e) {
            if (!sugerenciasContainer.contains(e.target) && e.target !== direccionInput) {
                sugerenciasContainer.style.display = 'none';
            }
        });
    }
    
    function buscarSugerenciasDireccion(query, container, input) {
        fetch(`/supervisor/api/pdv/sugerencias-direccion?q=${encodeURIComponent(query)}`)
            .then(response => response.json())
            .then(sugerencias => {
                container.innerHTML = '';
                ...
                sugerencias.forEach(sugerencia => {
                    ...
                    `;
                    item.textContent = sugerencia.direccion;
                    ...
                    item.addEventListener('click', function() {
                        ...
                        document.getElementById('latitud').value = sugerencia.latitud;
                        document.getElementById('longitud').value = sugerencia.longitud;
                        
                        // Actualizar ciudad si está disponible
                        if (sugerencia.ciudad) {
                            const selectCiudad = document.getElementById('ciudad');
                            const opcionExistente = Array.from(selectCiudad.options)
                                .find(option => option.text.toLowerCase() === sugerencia.ciudad.toLowerCase());
                            
                            if (opcionExistente) {
                                selectCiudad.value = sugerencia.ciudad;
                            } else {
                                // Agregar nueva opción
                                const nuevaOpcion = document.createElement('option');
                                nuevaOpcion.value = sugerencia.ciudad;
                                nuevaOpcion.textContent = sugerencia.ciudad;
                                selectCiudad.appendChild(nuevaOpcion);
                                selectCiudad.value = sugerencia.ciudad;
                            }
                        }
                        
                        // Actualizar departamento si está disponible
                        if (sugerencia.estado) {
                            const selectDepartamento = document.getElementById('departamento');
                            const opcionExistente = Array.from(selectDepartamento.options)
                                .find(option => option.text.toLowerCase() === sugerencia.estado.toLowerCase());
                            
                            if (opcionExistente) {
                                selectDepartamento.value = sugerencia.estado;
                        ...
                        // Actualizar el mapa
                        if (map && marker) {
                            const lat = parseFloat(sugerencia.latitud);
                            const lng = parseFloat(sugerencia.longitud);
                ...
                // Posicionar y mostrar el contenedor
    ...
    window.editarPDV = function(identificador) {
        fetch(`/supervisor/api/pdv/${identificador}`)
        .then(response => response.json())
        .then(pdv => {
            ...
            if (pdv.departamento) {
                fetch(`/supervisor/api/pdv/ciudades-por-departamento/${encodeURIComponent(pdv.departamento)}`)
                    .then(response => response.json())
                    .then(ciudades => {
            ...
            if (pdv.jerarquia_nivel_2) {
                fetch(`/supervisor/api/pdv/jerarquias-n2_2-por-n2/${encodeURIComponent(pdv.jerarquia_nivel_2)}`)
                    .then(response => response.json())
                    .then(jerarquias => {
...
function abrirModalNuevo() {
    ...
    // Recargar jerarquías N2_2 desde el servidor
    recargarJerarquiasN2_2();
    ...
    function guardarPDV() {
        ...
        const url = pdvId ? `/supervisor/api/pdv/${identificador}` : '/supervisor/api/pdv';
    ...
    function crearPDVFuerza(data, url, method) {
        ...
        // Esto requeriría un endpoint separado o un flag en la data
...
function agregarNuevoValorPDV() {
    ...
    switch(tipoActual) {
        ...
        case 'jerarquia_n2_2':
            ...
            endpoint = '/supervisor/api/pdv/jerarquias-n2-2';
            dataField = 'jerarquia';
            break;
    }
...
function recargarJerarquiasN2_2() {
    fetch('/supervisor/api/pdv/jerarquias-n2-2')
        .then(response => response.json())
        .then(data => {
            jerarquiasN2_2 = data;
        ...
        .catch(error => {
            console.error('Error recargando jerarquías N2_2:', error);
        });
}
});