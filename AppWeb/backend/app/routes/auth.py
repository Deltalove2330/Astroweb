...
def current_user_info():
    """Obtiene información del usuario actual - Combinado de ambas ramas"""
    
    # Base de datos común para todos los usuarios (de dev)
    user_data = {
        'id': current_user.id,
        'username': current_user.username,
        'rol': current_user.rol,
        'cliente_id': current_user.cliente_id,
        'id_analista': getattr(current_user, 'id_analista', None),
        'id_supervisor': getattr(current_user, 'id_supervisor', None),
        'email': getattr(current_user, 'email', None)
    }
    
    # Datos específicos para mercaderistas (de subida-fotos-completa)
    if current_user.rol == 'mercaderista':
        user_data['mercaderista_id'] = getattr(current_user, 'mercaderista_id', None)
    ...
    # Verificar si es supervisor y redirigir (de dev)
...
def verify_merchandiser():
    """Verificar mercaderista - Versión mejorada con Flask-Login (de subida-fotos-completa)"""
    try:
