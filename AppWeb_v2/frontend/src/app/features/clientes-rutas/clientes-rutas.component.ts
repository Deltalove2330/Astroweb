...
<div class="min-h-screen bg-slate-950 text-white">
  ...
  <div class="px-8 py-6 grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 max-w-6xl">
    ...
    <div class="bg-slate-900 border border-white/8 rounded-2xl overflow-hidden self-start">
      <div class="px-4 py-3 border-b border-white/8">
        <div class="relative">
          <mat-icon class="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500 !text-base">search</mat-icon>
          <input [(ngModel)]="filtro" placeholder="Buscar usuario cliente…"
            class="w-full bg-slate-800 border border-slate-700 focus:border-violet-500 rounded-xl pl-9 pr-3 py-2 text-sm outline-none">
        </div>
      </div>
      <div class="max-h-[68vh] overflow-y-auto">
        ...
        @for (u of usuariosFiltrados(); track u.id_usuario) {
          <button (click)="seleccionar(u)"
            class="w-full text-left px-4 py-3 border-b border-white/5 hover:bg-slate-800 transition-colors flex items-center gap-3"
            [class.bg-violet-950]="sel()?.id_usuario === u.id_usuario">
            <div class="w-9 h-9 rounded-xl bg-violet-900 flex items-center justify-center shrink-0"><mat-icon class="!text-base text-violet-400">person</mat-icon></div>
            <div class="min-w-0 flex-1">
              <p class="font-bold text-sm truncate">{{ u.cliente || u.username }}</p>
              <p class="text-xs text-slate-500 truncate">{{ u.username }}</p>
            </div>
            <span class="text-xs px-2 py-0.5 rounded-full font-bold shrink-0" [ngClass]="u.n_rutas ? 'bg-emerald-950 text-emerald-400' : 'bg-slate-800 text-slate-500'">{{ u.n_rutas }}</span>
          </button>
        }
        @if (!loadingUsers() && !usuariosFiltrados().length) { <p class="text-center text-slate-600 py-10 text-sm">Sin usuarios cliente.</p> }
      </div>
    </div>
      ...
      @if (!sel()) {
        ...
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-slate-400 text-sm">Rutas donde aparece este cliente en la programación</p>
          </div>
          <span class="text-sm text-slate-400">{{ asignadas() }} de {{ rutas().length }} asignadas</span>
        </div>
        ...
        @else {
          @for (r of rutas(); track r.id_ruta) {
            <div class="bg-slate-900 border rounded-2xl p-4 mb-3 flex items-center gap-3 transition-colors"
                 [ngClass]="r.asignada ? 'border-emerald-800/60' : 'border-white/8'">
              <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" [ngClass]="r.asignada ? 'bg-emerald-900' : 'bg-slate-800'">
                <mat-icon [ngClass]="r.asignada ? 'text-emerald-400' : 'text-slate-400'">route</mat-icon>
              </div>
          ...
          @if (!rutas().length) {
            <div class="bg-slate-900 border border-white/8 rounded-2xl py-12 text-center text-slate-500">
              <mat-icon class="!text-4xl block mx-auto mb-2 text-slate-700">wrong_location</mat-icon>
