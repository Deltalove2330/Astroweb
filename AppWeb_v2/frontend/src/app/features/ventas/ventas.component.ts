...
<div class="min-h-screen bg-slate-950 text-white">
  ...
  <div class="bg-gradient-to-r from-slate-900 to-slate-800 border-b border-white/8 px-6 py-5">
    <div class="flex items-center gap-3">
      ...
      <div class="flex-1 min-w-0">
        <h1 class="text-xl font-black tracking-tight leading-none">Ventas</h1>
        <p class="text-slate-400 text-xs mt-0.5">{{ crumb() || cedula }}</p>
      </div>
  ...
  <div class="px-6 py-6 max-w-3xl mx-auto pb-28">
    ...
    <!-- SIN JORNADA -->
    <div class="bg-slate-900 border border-white/8 rounded-2xl p-8 text-center mt-8">
      <mat-icon class="!text-5xl text-emerald-400">place</mat-icon>
      <h2 class="text-lg font-black mt-3 mb-1">¿Listo para trabajar?</h2>
      <p class="text-slate-400 text-sm mb-6">Activa tu ruta para comenzar a registrar tus visitas y ventas del día.</p>
      <button (click)="activarJornada()" class="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl font-black flex items-center justify-center gap-2">
    ...
    <!-- BARRA DE JORNADA -->
    <div class="bg-slate-900 border border-white/8 rounded-2xl p-4 mb-4 flex items-center justify-between gap-2">
      <div class="text-xs text-slate-400">
        <span class="text-emerald-400 font-bold"><mat-icon class="!text-sm align-middle">check_circle</mat-icon> Ruta activa</span><br>
        Iniciada: {{ fmtHora(jornadaActiva().fecha_inicio) }}
      </div>
      <div class="flex items-center gap-2">
        <span class="text-xs px-2 py-1 bg-emerald-950 text-emerald-300 rounded-full font-bold" title="Visitas registradas">{{ jornadaActiva().visitas || 0 }}</span>
        <button (click)="verVisitas()" class="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-slate-700"><mat-icon class="!text-base">receipt_long</mat-icon></button>
        <button (click)="finalizarJornada()" class="px-3 py-2 bg-red-950 hover:bg-red-900 text-red-300 rounded-lg text-xs font-bold">Finalizar</button>
      </div>
    </div>
    ...
    @if (step() === 'pdvs') {
      <div class="flex items-center justify-between mb-3">
        <div>
          <h2 class="text-lg font-black">Puntos de venta</h2>
          <p class="text-slate-400 text-sm">Selecciona el PDV que vas a visitar</p>
        </div>
        <button (click)="mostrarSolicitarPdv.set(!mostrarSolicitarPdv())" class="px-3 py-2 rounded-xl text-xs font-bold border border-emerald-700 text-emerald-400 flex items-center gap-1">
          <mat-icon class="!text-base">add_business</mat-icon> Solicitar PDV
        </button>
      </div>

      @if (mostrarSolicitarPdv()) {
        <div class="bg-slate-900 border border-emerald-900 rounded-2xl p-4 mb-4 space-y-2">
          <h3 class="font-bold text-emerald-400 text-sm mb-2">Solicitud de nuevo PDV</h3>
          <input [(ngModel)]="nuevoPdv.nombre" placeholder="Nombre del PDV *" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm outline-none">
          <input [(ngModel)]="nuevoPdv.rif" placeholder="RIF *" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm outline-none">
          <textarea [(ngModel)]="nuevoPdv.direccion" placeholder="Dirección completa *" rows="2" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm outline-none"></textarea>
          <label class="block text-xs text-slate-400">Foto de la tienda *</label>
          <input type="file" accept="image/*" capture="environment" (change)="onFotoTienda($event)" class="w-full text-xs">
          <label class="block text-xs text-slate-400">Foto del RIF *</label>
          <input type="file" accept="image/*" capture="environment" (change)="onFotoRif($event)" class="w-full text-xs">
          <div class="flex gap-2 pt-2">
            <button (click)="mostrarSolicitarPdv.set(false)" class="flex-1 py-2 border border-slate-700 rounded-lg text-sm font-bold text-slate-400">Cancelar</button>
            <button (click)="solicitarPdv()" [disabled]="enviandoSolicitud()" class="flex-1 py-2 bg-emerald-700 rounded-lg text-sm font-bold">Enviar solicitud</button>
          </div>
        </div>
      }

      <input [(ngModel)]="searchPdv" placeholder="Buscar PDV por nombre, ciudad..."
        class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-sm outline-none mb-3 focus:border-emerald-500">
      <p class="text-xs text-slate-500 mb-2">{{ filteredPdvs.length }} de {{ pdvs().length }} puntos de venta</p>
      <div class="max-h-[55vh] overflow-y-auto space-y-2">
        @for (p of filteredPdvs.slice(0, 100); track p.identificador) {
          <button (click)="seleccionarPdv(p)" class="w-full text-left bg-slate-900 hover:bg-slate-800 border border-white/8 rounded-xl p-3 flex items-center gap-3">
            <mat-icon class="text-emerald-400">storefront</mat-icon>
            <div class="flex-1 min-w-0">
              <p class="font-bold text-sm truncate">{{ p.nombre || p.identificador }}</p>
              <p class="text-xs text-slate-400 truncate">{{ pdvSub(p) }}</p>
            </div>
            <mat-icon class="text-slate-600">chevron_right</mat-icon>
          </button>
        }
        @if (!filteredPdvs.length) { <p class="text-center text-slate-600 py-12">No se encontraron puntos de venta</p> }
      </div>
    }
    ...
    @if (step() === 'clientes') {
      <div class="flex items-center gap-2 mb-3">
        <button (click)="step.set('pdvs')" class="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-slate-700"><mat-icon class="!text-base">arrow_back</mat-icon></button>
        <div>
          <h2 class="text-lg font-black">Clientes</h2>
          <p class="text-slate-400 text-xs">PDV: {{ pdvSel()?.nombre || pdvSel()?.identificador }}</p>
        </div>
      </div>
      ...
      <div class="max-h-[60vh] overflow-y-auto space-y-2">
        @for (c of filteredClientes; track c.id_cliente) {
          <button (click)="seleccionarCliente(c)" class="w-full text-left bg-slate-900 hover:bg-slate-800 border border-white/8 rounded-xl p-3 flex items-center gap-3">
            <mat-icon class="text-emerald-400">person</mat-icon>
            <p class="font-bold text-sm flex-1">{{ c.nombre }}</p>
            <mat-icon class="text-slate-600">chevron_right</mat-icon>
          </button>
        }
        @if (!filteredClientes.length) { <p class="text-center text-slate-600 py-12">No se encontraron clientes</p> }
      </div>
    }
    ...
    @if (step() === 'decision') {
      <div class="flex items-center gap-2 mb-4">
        <button (click)="step.set('clientes')" class="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-slate-700"><mat-icon class="!text-base">arrow_back</mat-icon></button>
        <div>
          <h2 class="text-lg font-black">{{ clienteSel()?.nombre }}</h2>
          <p class="text-slate-400 text-xs">{{ pdvSel()?.nombre }}</p>
        </div>
      </div>

      @if (vendio() === null) {
        <p class="text-center text-slate-300 font-semibold mb-4">¿Vendiste en este cliente?</p>
        <div class="flex gap-3">
          <button (click)="vendio.set(true)" class="flex-1 py-4 bg-emerald-700 hover:bg-emerald-600 rounded-xl font-black flex items-center justify-center gap-2">
            <mat-icon>check_circle</mat-icon> Sí, vendí
          </button>
          <button (click)="vendio.set(false)" class="flex-1 py-4 bg-red-800 hover:bg-red-700 rounded-xl font-black flex items-center justify-center gap-2">
            <mat-icon>cancel</mat-icon> No vendí
          </button>
        </div>
      } @else if (vendio() === true) {
        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase">Monto de la venta</label>
        <input type="number" [(ngModel)]="monto" min="0" step="0.01" placeholder="0.00"
          class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-lg outline-none mb-4 focus:border-emerald-500">
        <div class="flex gap-2">
          <button (click)="vendio.set(null)" class="flex-1 py-3 border border-slate-700 rounded-xl font-bold text-slate-400">Atrás</button>
          <button (click)="registrarVisita()" [disabled]="registrando()" class="flex-1 py-3 bg-emerald-700 rounded-xl font-black">Registrar venta</button>
        </div>
      } @else {
        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase">Motivo de la no venta</label>
        <textarea [(ngModel)]="razonNoVenta" rows="3" placeholder="Escribe la razón..."
          class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-sm outline-none mb-4 focus:border-emerald-500"></textarea>
        <div class="flex gap-2">
          <button (click)="vendio.set(null)" class="flex-1 py-3 border border-slate-700 rounded-xl font-bold text-slate-400">Atrás</button>
          <button (click)="registrarVisita()" [disabled]="registrando()" class="flex-1 py-3 bg-red-700 rounded-xl font-black">Registrar</button>
        </div>
      }
    }
