...
# ===================================================================
# DECORADOR DE ROL ÔÇö Cliente Encuestador (id_rol = 13)
# Admite tambi├®n a admin para inspecci├│n/QA.
# ===================================================================
...
# ===================================================================
# CONSTRUCCI├ôN DIN├üMICA DE WHERE A PARTIR DE FILTROS
# ===================================================================
def _multi(name):
...
def _build_filters():
    """
    Devuelve (where_sql, params) listos para concatenar despu├®s de
    ...
    # d├¡as_consulta = match parcial porque la columna es comma-separated
...
# ===================================================================
# CAT├üLOGOS DE FILTROS (poblar dropdowns del sidebar)
# ===================================================================
...
def api_filtros():
    try:
        ...
        return jsonify({
            ...
            "dias_consulta": ["Lunes", "Martes", "Mi├®rcoles", "Jueves",
                              "Viernes", "S├íbado", "Domingo"]
        })
...
def api_kpis():
    try:
        ...
        # Encuestas ├║ltimos 30 d├¡as (sobre la data filtrada)
        ...
        # Contactabilidad: % de m├®dicos (distintos) con cada canal
...
# ===================================================================
# HELPERS para gr├íficos: GROUP BY con filtros aplicados
# ===================================================================
...
def chart_pacientes_semana():
    order = ["1 a 5 pacientes", "6 a 10 pacientes", "11 a 15 pacientes",
             "16 a 20 pacientes", "21 a 30 pacientes", "M├ís de 30 pacientes"]
...
def chart_dias_consulta():
    """Conteo por d├¡a contando si aparece en dias_consulta o dias_consulta2."""
    where, params = _build_filters()
    days = ["Lunes", "Martes", "Mi├®rcoles", "Jueves", "Viernes", "S├íbado", "Domingo"]
    out = []
    for d in days:
        q = f"""
            SELECT COUNT(DISTINCT m.id_medico)
...
def chart_valor_por_especialidad():
    ...
    es texto, lo mapeamos a un punto medio num├®rico estimado.
    """
    where, params = _build_filters()
    q = f"""
...
def chart_heatmap_esp_estado():
    ...
    Cada celda = COUNT(DISTINCT m├®dicos).
    """
    where, params = _build_filters()
...
def chart_mapa_venezuela():
    """M├®dicos por estado para el mapa (Leaflet circle markers)."""
    where, params = _build_filters()
    q = f"""
...
@verificar_rol_cliente_encuestador
def chart_temporal():
    """Encuestas (con sus m├®dicos) por semana."""
    where, params = _build_filters()
    q = f"""
        SELECT
...
def chart_encuestadores_ranking():
    """Ranking de encuestadores: # m├®dicos y # centros aportados."""
    where, params = _build_filters()
    q = f"""
        SELECT TOP 15
...
# ===================================================================
# TABLA MAESTRA DE M├ëDICOS (paginada)
# ===================================================================
